...
out=root/('collect_xy_correction879_pick_'+datetime.now(timezone(timedelta(hours=9))).strftime('%Y%m%d_%H%M%S_%f_JST'))
...
def shared_load(args):
    ...
    xy_rows=[]
    xy_cache={}
    def capture_xy_features(module,inputs,value):
        xy_cache['hidden']=value.detach().cpu().numpy()[0].copy()
    model.fusion.register_forward_hook(capture_xy_features)
    def predict(obs,step):
        ...
        xy_rows.append((step,xy_cache['hidden'].copy(),original_action,action.copy()))
        if step==878:
            np.savez_compressed(output/'xy_correction_inputs.npz',
                steps=np.array([v[0] for v in xy_rows]),
                hidden=np.stack([v[1] for v in xy_rows]),
                policy_actions=np.stack([v[2] for v in xy_rows]),
                executed_actions=np.stack([v[3] for v in xy_rows]))
            print('XY_INPUTS_SAVED',len(xy_rows),flush=True)
        return action
    return predict
...
reference=root/'diagnose_xy_suppression800_pick_20260918_165105_878926_JST/pick/teacher_trace.npz'
with np.load(reference,allow_pickle=False) as a,np.load(dest/'teacher_trace.npz',allow_pickle=False) as b,np.load(dest/'xy_correction_inputs.npz',allow_pickle=False) as z:
    ...
    assert z['hidden'].shape==(879,256) and np.isfinite(z['hidden']).all()
    assert np.array_equal(z['steps'],np.arange(879))
    assert np.array_equal(z['executed_actions'],b['actions'])
    assert np.array_equal(z['policy_actions'][:800],z['executed_actions'][:800])
    assert np.array_equal(z['policy_actions'][:,2:],z['executed_actions'][:,2:])
    result={'collection_pass':True,'samples':879,'xy_corrected_samples':79,'trajectory_exactly_equal':same,'data_file':str(dest/'xy_correction_inputs.npz'),'data_sha256':hashlib.sha256((dest/'xy_correction_inputs.npz').read_bytes()).hexdigest(),'training_performed':False,'vla_success':False,'scope':'First800 policy outputs retained; last79 XY command intervention targets. Other components unchanged. Data collection only, not learned autonomous success.'}
(out/'results.json').write_text(json.dumps(result,indent=2))
print('XY_CORRECTION_INPUT_COLLECTION_RESULT',json.dumps(result),flush=True)
