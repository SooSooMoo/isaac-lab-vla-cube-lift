...
out=root/('record_final_pick_'+datetime.now(timezone(timedelta(hours=9))).strftime('%Y%m%d_%H%M%S_%f_JST'))
...
def shared_load(args):
    ...
    def record_frame(captured,step):
        ...
        if video_process is None:
            ...
            video_process=video_subprocess.Popen([imageio_ffmpeg.get_ffmpeg_exe(),'-hide_banner','-loglevel','error','-y','-f','rawvideo','-pix_fmt','rgb24','-s',str(width)+'x'+str(height),'-r','50','-i','pipe:0','-an','-c:v','libx264','-preset','fast','-crf','18','-pix_fmt','yuv420p','-movflags','+faststart',str(output/'policy_view.mp4')],stdin=video_subprocess.PIPE,stderr=video_subprocess.DEVNULL)
        ...
        if step==878:
            ...
            print('VIDEO_SAVED',str(output/'policy_view.mp4'),video_frames,flush=True)
...
with np.load(reference,allow_pickle=False) as a,np.load(dest/'teacher_trace.npz',allow_pickle=False) as b:
    assert len(b['actions'])==879
    same={k:bool(np.array_equal(a[k],b[k])) for k in ('actions','pre_eef','post_eef','pre_quat_xyzw','post_quat_xyzw','post_cube','gripper_pos')}
import imageio_ffmpeg
video=dest/'policy_view.mp4'
assert video.is_file() and video.stat().st_size>0
reader=imageio_ffmpeg.read_frames(str(video),pix_fmt='rgb24')
metadata=next(reader);decoded=sum(1 for frame in reader)
assert decoded==879,('Wrong video frame count',decoded)
result={'task':'pick','trajectory_exactly_equal':same,'recording_comparison_pass':all(same.values()),'video_file':str(video),'video_bytes':video.stat().st_size,'decoded_frames':decoded,'fps':metadata['fps'],'view':'left table camera; right wrist camera','video_speed':'real time: 50fps, simulation dt0.02s','visual_quality_verified':False,'scope':'Unedited policy-input camera views, no frame cropping; each frame is pre-action, final post-action state is not included. Numerical outcomes remain in trace.'}
(out/'video_result.json').write_text(json.dumps(result,indent=2))
print('VIDEO_RECORDING_RESULT',json.dumps(result),flush=True)
PY
...
out=root/('record_final_above_'+datetime.now(timezone(timedelta(hours=9))).strftime('%Y%m%d_%H%M%S_%f_JST'))
...
reader=imageio_ffmpeg.read_frames(str(video),pix_fmt='rgb24')
metadata=next(reader);decoded=sum(1 for frame in reader)
assert decoded==424,('Wrong video frame count',decoded)
result={'task':'above','trajectory_exactly_equal':same,'recording_comparison_pass':all(same.values()),'video_file':str(video),'video_bytes':video.stat().st_size,'decoded_frames':decoded,'fps':metadata['fps'],'view':'left table camera; right wrist camera','video_speed':'real time: 50fps, simulation dt0.02s','visual_quality_verified':False,'scope':'Unedited policy-input camera views, no frame cropping; each frame is pre-action, final post-action state is not included. Numerical outcomes remain in trace.'}
(out/'video_result.json').write_text(json.dumps(result,indent=2))
print('VIDEO_RECORDING_RESULT',json.dumps(result),flush=True)
PY
