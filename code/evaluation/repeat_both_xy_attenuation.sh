...
out=root/('repeat_xy_attenuation_pick_'+datetime.now(timezone(timedelta(hours=9))).strftime('%Y%m%d_%H%M%S_%f_JST'))
...
reference_repeat=root/'verify_xy_attenuation1300_pick_20260918_173544_857706_JST/pick/teacher_trace.npz'
with np.load(reference_repeat,allow_pickle=False) as previous,np.load(dest/'teacher_trace.npz',allow_pickle=False) as current:
    replay={k:bool(np.array_equal(previous[k],current[k])) for k in ('actions','pre_eef','post_eef','pre_quat_xyzw','post_quat_xyzw','post_cube','gripper_pos')}
result['repeatability_pass']=all(replay.values())
result['repeat_exactly_equal']=replay
print('TASK_REPEAT_RESULT',json.dumps({'task':'pick','numerical_pass':result.get('above_verified_success',result.get('lift_and_horizontal_proxy_pass',False)),'repeatability_pass':all(replay.values()),'trajectory_exactly_equal':replay,'result_file':str(out/'results.json')}),flush=True)
...
out=root/('repeat_xy_attenuation_above_'+datetime.now(timezone(timedelta(hours=9))).strftime('%Y%m%d_%H%M%S_%f_JST'))
...
initial=json.loads((dest/'initial_input_check.json').read_text());assert initial['pass']
summary=json.loads((dest/'summary.json').read_text())
with np.load(dest/'teacher_trace.npz',allow_pickle=False) as b:
    n=len(b['actions']);assert n>0
    eef=b['post_eef'];cube=b['post_cube'];fingers=b['gripper_pos']
    for v in (eef,cube,fingers,b['actions']):assert np.isfinite(v).all()
    err=np.linalg.norm(eef-cube-np.array([0,0,.1]),axis=1)
    valid=(err<=.03)&(fingers[:,0]>=.03)&(fingers[:,1]<=-.03)
    current=best=0
    for v in valid:
        current=current+1 if v else 0;best=max(best,current)
    required=int(summary['required_above_hold_steps'])
    result={'samples':n,'initial_input_pass':True,'reported_success':bool(summary['success']),'above_verified_success':bool(summary['success'] and best>=required),'required_hold_steps':required,'max_hold_steps':best,'position_limit_mm':30,'final_above_error_mm':float(err[-1]*1000),'closest_above_error_mm':float(err.min()*1000),'closed_command_count':int((b['actions'][:,6]<0).sum()),'stop_reason':summary['stop_reason'],'training_performed':False,'runtime_step_based_override':False,'checkpoint_sha256':digest,'adapter_sha256':hashlib.sha256(adapter_path.read_bytes()).hexdigest(),'gripper_correction_sha256':'48fc3c58dea3deaef9fbae6d1208d7df522967ff841162f5077f88eab0143863','xy_attenuation_sha256':'0f5b74efb4c9f34be3a9ab6b8985117869d8e6e1ff9565a1ab277f88ec2e3de5','scope':'Same model stack as numerical pick success. Above numerical verification; visual quality and independent repeatability not established.'}
assert hashlib.sha256(checkpoint.read_bytes()).hexdigest()==digest
reference_repeat=root/'verify_xy_attenuation_above_20260918_174047_971979_JST/above/teacher_trace.npz'
with np.load(reference_repeat,allow_pickle=False) as previous,np.load(dest/'teacher_trace.npz',allow_pickle=False) as current:
    replay={k:bool(np.array_equal(previous[k],current[k])) for k in ('actions','pre_eef','post_eef','pre_quat_xyzw','post_quat_xyzw','post_cube','gripper_pos')}
result['repeatability_pass']=all(replay.values())
result['repeat_exactly_equal']=replay
print('TASK_REPEAT_RESULT',json.dumps({'task':'above','numerical_pass':result.get('above_verified_success',result.get('lift_and_horizontal_proxy_pass',False)),'repeatability_pass':all(replay.values()),'trajectory_exactly_equal':replay,'result_file':str(out/'results.json')}),flush=True)
(out/'results.json').write_text(json.dumps(result,indent=2))
print('XY_ATTENUATION_ABOVE_RESULT',json.dumps(result),flush=True)
print('RESULT_FILE',out/'results.json',flush=True)
PY
