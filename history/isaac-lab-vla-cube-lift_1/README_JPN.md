...
この旧版では、固定の指示をデータセットのメタデータに保存していますが、方策モデルへの言語入力には使用していません。完全版HDF5は、容量を確認したうえでGitHub Releasesまたは外部ストレージへ配置します。
...
Isaac LabおよびRobomimicのライセンス条件を確認してください。この旧版（Version 1）のライセンスは、同じディレクトリの [LICENSE](LICENSE) を参照してください。
